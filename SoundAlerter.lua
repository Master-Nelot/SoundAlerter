...
if ((GetLocale() == "zhCN") or (GetLocale() == "zhTW") or (GetLocale() == "koKR") or (GetLocale() == "frFR")) then
DEFAULT_CHAT_FRAME:AddMessage("|cffFF7D0ASoundAlerter|r Данная модификация адаптирована под Sirus.su, и работает только на русском клиенте. Со всеми предложениями обращаться в дискорд |cff0070DEhttps://discord.gg/7cjU9xvcQY|r");
end
...
function SoundAlerter:OnInitialize()
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cffFF7D0ASoundAlerter|r by |cff0070DETrolollolol|r Адаптировал под Sirus |cff0070DEМастер|r  Окно настроек /sa ");
...
function SoundAlerter:spellOptions(order, spellID, ...)
	...
	self:Print("<SA>: Ошибка загрузки ID заклинания " ..spellID .. " как будто его не существует. Проверьте, правильная ли у вас версия SoundAlerter")
	end
end
...
function SoundAlerter:PlaySpell(list, spellID, ...)
	if list[spellID] then
		...
		if sadb.debugmode then print("<SA> DEBUG: Воспроизводится: "..list[spellID]..".mp3"); end
			...
			if ((spellID == 42292) and sadb.trinketalert) then
				if ((currentZoneType == "arena" or pvpType == "arena") or (sourceuid.target or sourceuid.focus)) then
					if (self:ArenaClass(sourceGUID) and sadb.class) then
						PlaySoundFile(sadb.sapath..sadb.class..".mp3"); --todo: convert ogg class to mp3
...
local DRINK_SPELL = GetSpellInfo(57073 or 43183)
