--[[SoundAlerter by Trolollolol If you have any issues or concerns with the addon, Send me an ingame message at Trolollol - Realm:Sargeras Server:Molten-WoW.com]]
...
SA_LOCALEPATH = {
	ruRU = "Interface\\Addons\\SoundAlerter\\Voice_Milena_ru\\",
}
self.SA_LOCALEPATH = SA_LOCALEPATH
local SA_LANGUAGE = {
	["Interface\\Addons\\SoundAlerter\\Voice_Milena_ru\\"] = L["Milena_ru"],
}
...
function SoundAlerter:OnInitialize()
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cffFF7D0ASoundAlerter|r by |cff0070DETrolollolol|r Адаптация под Sirus |cff0070DEMaster-f1|r  Окно настроек /sa ")
...
function SoundAlerter:spellOptions(order, spellID, ...)
	...
	if spellname ~= nil then
		...
		self:Print("<SA>: Ошибка загрузки ID заклинания " ..spellID)
	end
end
...
function SoundAlerter:PlaySpell(list, spellID, ...)
	if list[spellID] then
		...
		if sadb.debugmode then
			print("<SA> DEBUG: Воспроизводится: "..list[spellID]..".mp3")
		end
...
function SoundAlerter:COMBAT_LOG_EVENT_UNFILTERED(event, ...)
	...
	if (not ((pvpType == "contested" and sadb.field) or (pvpType == "hostile" and sadb.field) or (pvpType == "friendly" and sadb.field) or 
		(currentZoneType == "pvp" and sadb.battleground) or (((currentZoneType == "arena") or (pvpType == "arena")) and sadb.arena) or sadb.all)) then
		return
	end
	...
	if sadb.debugmode and spellName == sadb.csname and spellName then
		print("Ваше заклинание: "..spellName,spellID,event,sourceName,destName)
	end
	--Special conditions (freezing trap)
	if desttype[COMBATLOG_FILTER_HOSTILE_PLAYERS] and event == "SPELL_CREATE" and (spellID == 13809 or spellID == 13810 or spellID == 1499) and 
		((sadb.myself and (destuid.target or destuid.focus)) or sadb.enemyinrange) then
	...
	if (event == "SPELL_AURA_APPLIED") then
		if desttype[COMBATLOG_FILTER_HOSTILE_PLAYERS] then
			if sourcetype[COMBATLOG_FILTER_ME] then --my spells against enemy
				...
				if not sadb.chatalerts then
					if (((spellID == 6770 or spellID == 11297 or spellID == 51724) and sadb.sapenemy) or (spellID == 2094 and sadb.blindenemy) or 
						(spellID == 33786 and sadb.cycloneenemy) or (spellID == 51514 and sadb.hexenemy) or (spellID == 5782 and sadb.fearenemy)) then
		...
		elseif desttype[COMBATLOG_FILTER_ME] then --spells applied to myself
			if not sadb.chatalerts then --todo: Получите локализованную таблицу имен заклинаний, чтобы сравнить вместо этого/CSS пользовательские чаты
				if sourcetype[COMBATLOG_FILTER_HOSTILE_PLAYERS] or ((spellID == 6770 or spellID == 11297 or spellID == 51724) and sadb.sapselffriend) then
					if ((spellID == 51514 and sadb.hexselffriend) or (spellID == 33786 and sadb.cycloneselffriend) or 
						((spellID == 6215 or spellID == 17928 or spellID == 5484) and sadb.fearselffriend) or 
						((spellID == 12826 or spellID == 118 or spellID == 28271 or spellID == 28272) and sadb.polyenemy) or (spellID == 2094 and sadb.blindselffriend)) then
		...
		elseif desttype[COMBATLOG_FILTER_FRIENDLY_UNITS] then --Оповещения в чат, враг произнес заклинание на друга
			if (not sadb.chatalerts and not desttype[COMBATLOG_FILTER_ME] and (destuid.target or destuid.focus or (currentZoneType == "arena" or pvpType == "arena"))) then
				...
				elseif ((spellID == 51514 and sadb.hexselffriend) or (spellID == 33786 and sadb.cycloneselffriend) or 
					((spellID == 6215 or spellID == 17928 or spellID == 5484) and sadb.fearselffriend) or
					((spellID == 12826 or spellID == 118 or spellID == 28271 or spellID == 28272) and sadb.polyenemy) or (spellID == 2094 and sadb.blindselffriend)) then
	...
	elseif (event == "SPELL_AURA_REMOVED" and desttype[COMBATLOG_FILTER_HOSTILE_PLAYERS] and ((sourcetype[COMBATLOG_FILTER_ME] or (destuid.target or destuid.focus)))) then
		...
		if (not sadb.chatalerts and ((sourcetype[COMBATLOG_FILTER_ME] and sadb.chatdownself) or 
			((not sadb.caonlyTF or destuid.target or destuid.focus) and sadb.chatdownfriend))) then
			if ((spellID == 33786 and sadb.cycloneenemy) or (spellID == 51514 and sadb.hexenemy) or (spellID == 2094 and sadb.blindenemy) or
				...
				SendChatMessage(GetSpellLink(spellID).." спал с "..destName)
			end
		end
	elseif (event == "SPELL_CAST_SUCCESS") then
		if sourcetype[COMBATLOG_FILTER_HOSTILE_PLAYERS] then
			if (not sadb.chatalerts) then
				if ((sadb.vanishenemy and spellID == 26889) or (sadb.stealthenemy and spellID == 1784) or 
					(sadb.bubbleenemy and spellID == 642)) and ((sourceuid.target or sourceuid.focus) or 
					((sadb.vanishTF and spellID == 26889) or (sadb.stealthTF and spellID == 1784) or (sadb.bubbleTF and spellID == 642))) then
		...
		elseif (desttype[COMBATLOG_FILTER_FRIENDLY_UNITS] and not desttype[COMBATLOG_FILTER_ME] and 
			((destuid.target or destuid.focus) or (currentZoneType == "arena" or pvpType == "arena")) and not sadb.dArenaPartner) then
	...
	for k,css in pairs (sadb.custom) do
		...
		if sadb.debugmode and css.name == sadb.cspell and (spellID == tonumber(css.spellid) or (css.acceptSpellName and (css.spellname == spellName))) then
			log(css.name..": event: "..(css.eventtype[event] and "true" or "false")..", actual event: "..event..", dest spell: "..(destuid[css.destuidfilter] 
			and "true" or "false")..", dest type: "..(desttype[css.desttypefilter] and "true" or "false")..", sourceunit: "..(sourceuid[css.sourceuidfilter] 
			and "true" or "false")..", source type: "..(sourcetype[css.sourcetypefilter] and "true" or "false"))
		end
		if css.eventtype[event] and destuid[css.destuidfilter] and desttype[css.desttypefilter] and sourceuid[css.sourceuidfilter] and 
			sourcetype[css.sourcetypefilter] and (spellID == tonumber(css.spellid) or (css.acceptSpellName and (css.spellname == spellName))) then
			if sadb.debugmode then
				self:Print("Воспроизводится "..css.name)
			end
...
local DRINK_SPELL = {43183, 57073, 45548, 45543, 45544, 317844}

function SoundAlerter:UNIT_AURA(event,uid)
	if ((currentZoneType == "arena") or (pvpType == "arena")) and sadb.drinking then
		for _,v in pairs(DRINK_SPELL) do
			local spellID = GetSpellInfo(v)
			if UnitAura(uid,spellID) then
